(() => {
    ...
    // Safely get the top-level container
    ...
    if (!document.getElementById("wp-global-style")) {
        ...
        globalStyles.href = chrome.runtime.getURL("overlay.css");
    ...
    const IC = {
        ...
        skipBack: `<svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true"><path d="M12 5V2L8 6l4 4V7c2.76 0 5 2.24 5 5s-2.24 5-5 5-5-2.24-5-5H5c0 3.87 3.13 7 7 7s7-3.13 7-7-3.13-7-7-7z"/><text x="12" y="15" text-anchor="middle" font-size="5.5" font-family="system-ui,sans-serif" font-weight="800">10</text></svg>`,
        skipFwd: `<svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true"><path d="M18 13c0 3.31-2.69 6-6 6s-6-2.69-6-6 2.69-6 6-6v3l4-4-4-4v3c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8h-2z"/><text x="12" y="15" text-anchor="middle" font-size="5.5" font-family="system-ui,sans-serif" font-weight="800">10</text></svg>`,
    ...
    function addPlayerButton(video) {
        const targetContainer = getRootContainer();
        if (!targetContainer) return; // Fail safely if document is entirely inaccessible
        ...
        btn.addEventListener("click", e => {
            try {
                ...
                console.error("[WebPlayer] Launch failed:", err);
            }
        });
    ...
    function injectCustomPlayer(video) {
        ...
        video.dataset.originalTransform = video.style.transform || "";
        video.dataset.originalFilter = video.style.filter || "";
        ...
        uiWrapper.innerHTML = `
            ...
            <div class="wp-center-row">
                <button id="wp-skip-back" title="Back 10s" aria-label="Back 10 seconds">${IC.skipBack}</button>
                <button id="wp-play" title="Play / Pause" aria-label="Play or pause">${IC.play}</button>
                <button id="wp-skip-fwd" title="Forward 10s" aria-label="Forward 10 seconds">${IC.skipFwd}</button>
                ...
                <button id="wp-pip" title="Picture in Picture" aria-label="Picture in Picture">${IC.pip}</button>
                <button id="wp-fs" title="Fullscreen" aria-label="Toggle fullscreen">${IC.fullscreen}</button>
                <button id="wp-rotate" title="Rotate video" aria-label="Rotate video">${IC.rotate}</button>
                <button id="wp-exit" title="Exit WebPlayer" aria-label="Exit WebPlayer">${IC.close}</button>
            </div>
        `;
        ...
        const preventTouch = (e) => { if (document.fullscreenElement) e.preventDefault(); };
        ...
        let isTracking = true, rafId = null;

        function trackVideoPosition() {
            if (!isTracking) return;
            try {
                const targetContainer = document.fullscreenElement || getRootContainer();
                ...
                if (document.fullscreenElement) {
                    gestureZone.style.left = "0px"; gestureZone.style.top = "0px";
                    gestureZone.style.width = `${window.innerWidth}px`; gestureZone.style.height = `${window.innerHeight}px`;
                } else {
                    gestureZone.style.left = `${rect.left}px`; gestureZone.style.top = `${rect.top}px`;
                    gestureZone.style.width = `${rect.width}px`; gestureZone.style.height = `${rect.height}px`;
                }
                ...
                uiX = Math.max(uiWrapper.offsetWidth / 2 + 10, Math.min(uiX, window.innerWidth - uiWrapper.offsetWidth / 2 - 10));
                uiY = Math.max(10, Math.min(uiY, window.innerHeight - uiWrapper.offsetHeight - 10));
                uiWrapper.style.left = `${uiX}px`; uiWrapper.style.top = `${uiY}px`;

                let feedX = rect.left + rect.width / 2;
                let feedY = rect.top + rect.height * 0.15;
                feedX = Math.max(feedbackOverlay.offsetWidth / 2 + 10, Math.min(feedX, window.innerWidth - feedbackOverlay.offsetWidth / 2 - 10));
                feedY = Math.max(10, Math.min(feedY, window.innerHeight - feedbackOverlay.offsetHeight - 10));
                feedbackOverlay.style.left = `${feedX}px`; feedbackOverlay.style.top = `${feedY}px`;
            } catch (_) {}
        ...
        function showFeedback(text, keepAlive = false) {
            ...
            if (!keepAlive) {
                ...
                feedbackTimer = setTimeout(() => { if (feedbackOverlay) feedbackOverlay.style.opacity = "0"; }, 800);
            }
        }

        function setBuffering(on) { spinner?.classList.toggle("wp-buffering", on); }

        const videoListeners = {};
        function cleanup() {
            isTracking = false;
            if (rafId) cancelAnimationFrame(rafId);
            clearTimeout(feedbackTimer); clearTimeout(hideTimer);
            try {
                ...
                video.style.transform = video.dataset.originalTransform;
                video.style.filter = video.dataset.originalFilter;
                
                if (video.style) video.style.pointerEvents = video.dataset.originalPointerEvents || "";
                
                const root = getRootContainer();
                if (root) root.classList.remove("webplayer-active");

                Object.entries(videoListeners).forEach(([evt, fn]) => video.removeEventListener(evt, fn));
                document.removeEventListener("fullscreenchange", onFullscreenChange);
                window.removeEventListener("dblclick", globalEventShield, true);

                gestureZone.remove(); spinner.remove(); uiWrapper.remove(); feedbackOverlay.remove();
                window.removeEventListener("mouseup", mouseUpGlobalListener);
                window.removeEventListener("touchend", touchEndGlobalListener);

                addPlayerButton(video);
            } catch (e) { console.warn("[WebPlayer] Cleanup error:", e); }
        }

        uiWrapper.querySelector("#wp-exit")?.addEventListener("click", cleanup);

        function onFullscreenChange() { requestAnimationFrame(trackVideoPosition); showControls(); }
        ...
        videoListeners.loadedmetadata = () => {
            try { safeUpdateText(timeDur, Number.isFinite(video.duration) ? formatTime(video.duration) : "Live"); } catch (_) {}
        };
        ...
        videoListeners.error = () => {
            ...
            showFeedback(msgs[video.error?.code] || "Video error", true);
        };
        ...
        if ('mediaSession' in navigator) {
            ...
            navigator.mediaSession.setActionHandler('seekforward', (e) => safeSeekForward(video, e.seekOffset || 10));
        }

        const playBtn = uiWrapper.querySelector("#wp-play");
        videoListeners.play  = () => { if (playBtn) playBtn.innerHTML = IC.pause; };
        videoListeners.pause = () => { if (playBtn) playBtn.innerHTML = IC.play; showControls(); };
        video.addEventListener("play",  videoListeners.play);
        video.addEventListener("pause", videoListeners.pause);

        playBtn?.addEventListener("click", () => { try { video.paused ? video.play() : video.pause(); } catch (_) {} });
        uiWrapper.querySelector("#wp-skip-back")?.addEventListener("click", () => { try { video.currentTime = Math.max(0, video.currentTime - 10); } catch (_) {} showFeedback("−10s"); });
        uiWrapper.querySelector("#wp-skip-fwd")?.addEventListener("click", () => { safeSeekForward(video, 10); showFeedback("+10s"); });
        uiWrapper.querySelector("#wp-speed")?.addEventListener("change", e => { try { video.playbackRate = parseFloat(e.target.value); } catch (_) {} showFeedback(`${e.target.value}×`); });
        uiWrapper.querySelector("#wp-pip")?.addEventListener("click", async () => { try { document.pictureInPictureElement ? await document.exitPictureInPicture() : await video.requestPictureInPicture(); } catch (_) { showFeedback("PiP unavailable"); } });
        uiWrapper.querySelector("#wp-fs")?.addEventListener("click", () => { try { document.fullscreenElement ? document.exitFullscreen() : (wpShell.requestFullscreen?.() ?? video.requestFullscreen?.()); } catch (_) {} });
        ...
        let pressTimer = null, isLongPressing = false, isPointerDown = false, gestureActionTaken = false, originalSpeed = 1.0;
        let startX = 0, startY = 0, lastY = 0, lastTapTime = 0, lastTapX = 0, swipeDirection = null, currentBrightness = 1.0, gestureThrottleTimer = null;

        function createRipple(x, y) {
            const ripple = document.createElement("div"); ripple.className = "wp-ripple";
        ...
        function handlePointerDown(e) {
            try {
                ...
                isPointerDown = true; gestureActionTaken = false; swipeDirection = null;
                gestureZone.setPointerCapture(e.pointerId);
                startX = e.clientX; startY = e.clientY; lastY = e.clientY; originalSpeed = video.playbackRate;
                pressTimer = setTimeout(() => { isLongPressing = true; gestureActionTaken = true; try { video.playbackRate = 2.0; } catch (_) {} showFeedback("2× Speed", true); }, 500);
            } catch (_) {}
        }
        ...
        function handlePointerMove(e) {
            try {
                ...
                if (swipeDirection === "vertical" && !gestureThrottleTimer) {
                    const videoMidX = gestureZone.getBoundingClientRect().left + gestureZone.offsetWidth / 2;
        ...
        function handlePointerUp(e) {
            try {
                ...
                isPointerDown = false; try { gestureZone.releasePointerCapture(e.pointerId); } catch (_) {}
                clearTimeout(pressTimer);

                const diffX = e.clientX - startX, zoneRect = gestureZone.getBoundingClientRect(), relativeX = e.clientX - zoneRect.left;
                if (isLongPressing) { try { video.playbackRate = originalSpeed; } catch (_) {} isLongPressing = false; if (feedbackOverlay) feedbackOverlay.style.opacity = "0"; return; }
                if (swipeDirection === "vertical") { if (feedbackOverlay) feedbackOverlay.style.opacity = "0"; return; }
                ...
                if (!gestureActionTaken) {
                    ...
                    if (currentMs - lastTapTime < 350 && Math.abs(e.clientX - lastTapX) < 40) {
                        createRipple(e.clientX, e.clientY - zoneRect.top);
                        if (relativeX < zoneRect.width * 0.33) { video.currentTime = Math.max(0, video.currentTime - 10); showFeedback("−10s"); showControls(); }
                        else if (relativeX > zoneRect.width * 0.66) { safeSeekForward(video, 10); showFeedback("+10s"); showControls(); }
                        else { try { video.paused ? (video.play(), showFeedback("Play")) : (video.pause(), showFeedback("Pause")); } catch (_) {} }
                        lastTapTime = 0;
                    } else { lastTapTime = currentMs; lastTapX = e.clientX; }
                }
            } catch (_) {}
        }
